...
public class SpotifyAuthorization {
    ...
    public static Token get() throws Exception {
        ...
        Token token = gson().fromJson(responseOf(request(client_id, client_secret)), Token.class);
        token.updateExpiration();
        return token;
    }

    private static HttpRequest request(String client_id, String client_secret) {
        return HttpRequest.newBuilder()
                .uri(URI.create(ACCOUNT_BASE_URI))
                .header("Authorization", "Basic " + id(client_id, client_secret))
                .header("Content-Type", "application/x-www-form-urlencoded")
                .POST(HttpRequest.BodyPublishers.ofString("grant_type=client_credentials"))
                .build();
    }

    private static String id(String client_id, String client_secret) {
